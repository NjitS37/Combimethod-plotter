...
# Constants
hashlist = "mangacleartext.txt"
rockyoupcfg = "Largerockyou.txt"  # PCFG Rockyou list of length end_loop - step
rockyou = "rockyou.txt"
wordlist = "mangascrape.txt"
llamalist = "mangallama3110000_results.txt"
philist = "mangaphi4.txt"
hashcat_mode = "99999"
attack_mode = "0"
outfile = "cracked.txt"
plot_filename = "mangacombiresults.png"  # The saved plot filename
rule_name = "OneRuleToRuleThemStill.rule"
N = 620335 # Lengte van woordnelijst

# Storage for results
x_values = [0]  # Wordlist size
y_values = [0]  # Passwords cracked

xplotvalues = [] # list to keep track of amount cracked
yplotvalues = [] # list to keep track of amount cracked
total_cracked = 0
...
for lines in range(start_rockyou, end_rockyouloop, step_rockyou):
    ...
    xplotvalues.append(lines)
    yplotvalues.append(cracked_count)
...
for lines in range(start_scrape, end_scrapeloop, step_scrape):
    ...
    xplotvalues.append(maxpcfgx +1000*lines)
    yplotvalues.append(cracked_count)
...
for lines in range(start_scrape, end_scrapeloop, step_scrape):
    ...
    xplotvalues.append(maxscrapex +1000*lines)
    yplotvalues.append(cracked_count)
...
for lines in range(start_scrape, end_scrapeloop, step_scrape):
    ...
    xplotvalues.append(maxllamax +1000*lines)
    yplotvalues.append(cracked_count)
...
plt.plot(xplotvalues, yplotvalues, marker='o', linestyle='-', color='b', label='Number of cracked')
"""
...
plt.title("Hashcat cracking performance for multiple methods for the Mangatraders leak")
plt.grid(True)
plt.legend()
